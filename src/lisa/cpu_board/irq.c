    ...
    for (i = 1; i < 9; i++) // search timers in all VIA's
        if (via[i].active)  // which are marked as active.
        {
            ...
            if (!!(via[i].via[IER] & via[i].via[IFR]))
            {
                ...
                // address (seen with MacWorks Plus II reading 0xFCE01A with
                // a COPS interrupt pending). End the time slice instead so
                // the pending interrupt is taken after the instruction.
